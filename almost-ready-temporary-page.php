...
require_once ARTP_PLUGIN_DIR . 'includes/class-settings-page.php';
 ...
 *
 * Shows "Edit Page" when the temporary page is active (published), and
 * "Settings" when it is inactive so there is always a clear path to re-enable.
 *
...
function artp_add_action_links( $links ) {
	...
	if ( $page && 'publish' === $page->post_status ) {
		...
		$primary_link = sprintf(
			'<a href="%s">%s</a>',
			esc_url( ARTP_Settings_Page::get_settings_url() ),
			esc_html__( 'Settings', 'almost-ready-temporary-page' )
		);
	}

	array_unshift( $links, $primary_link );
